...
os: macOS
status: fixed
investigation_notes: "Archive completed cards incorrectly includes cancelled [-] and in-progress [/] tasks. Root cause: when lane.data.shouldMarkItemsComplete was true, ALL items in that lane were archived regardless of status. Fixed by removing the shouldMarkItemsComplete check from archive filter - now only items with checkChar matching getTaskStatusDone() (typically 'x') are archived."
fix_attempted: true
fix_pr: ""
---

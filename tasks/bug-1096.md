---
issue: 1096
title: error message in console on renaming kanban board
url: https://github.com/mgmeyers/obsidian-kanban/issues/1096
author: dictionarymouse
created: 2025-01-23T14:58:01Z
os: macOS
status: fixed
investigation_notes: "Console error 'handleRename is not a function' when renaming kanban board. The rename event handler called handleRename on all kanban leaves without checking if the view was fully initialized. Fix: add check for handleRename function existence before calling."
fix_attempted: true
fix_pr: ""
---

## Description

### Describe the bug

Console error message when renaming the kanban board from the tab.



### Expected Behavior

No error message?
It seems to update the name just fine. It seems a mild issue, though I have no links to that particular kanban board, yet, so I don't necessarily know the impact.

### Steps to reproduce

Click the tab of the kanban board, rename it. Error message appears.

### Relevant errors (if available)

```shell
app.js:1 Uncaught TypeError: o.view.handleRename is not a function
    at eval (plugin:obsidian-kanban:67:14164)
    at Array.forEach (<anonymous>)
    at eval (plugin:obsidian-kanban:67:14145)
    at e.tryTrigger (app.js:1:723006)
    at e.trigger (app.js:1:722939)
    at t.trigger (app.js:1:741044)
    at t.onChange (app.js:1:731667)
    at e.trigger (app.js:1:545565)
    at e.<anonymous> (app.js:1:536409)
    at app.js:1:237228
```

### Operating system

macOS

## Investigation Notes
<!-- To be filled during triage -->

## Fix Attempts
<!-- Document any attempted fixes -->

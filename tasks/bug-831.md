---
issue: 831
title: multi date tag colors [ between now and [x] before now ] longest x overide other x's color
url: https://github.com/mgmeyers/obsidian-kanban/issues/831
author: xtmu
created: 2023-09-25T13:33:02Z
os: Windows
status: verified
investigation_notes: "Date color priority issue - longer time spans override shorter ones. When multiple date color rules apply (1 day, 1 week, 1 month), the logic doesn't properly prioritize more specific rules. Need to check date color matching order in card rendering."
fix_attempted: false
fix_pr: ""
---

## Description

### Describe the bug

If I create multi tag for [ between now and [x] **before** now ] that x differs from 1 day,1 week, 1 month, then the 1 day and 1 week date tag color will be the same with 1 month tag color. 


### Expected Behavior

_No response_

### Steps to reproduce

Create date tag colors with different time span in [ between now and [x] before now ]
```json
  "date-colors": [
    {
      "distance": 3,
      "unit": "days",
      "direction": "after",
      "isAfter": true,
      "backgroundColor": "rgba(112, 255, 115, 0.12)"
    },
    {
      "distance": 3,
      "unit": "months",
      "direction": "after",
      "backgroundColor": "rgba(112, 255, 115, 0.4)"
    },
    {
      "isToday": false,
      "distance": 1,
      "unit": "months",
      "direction": "after",
      "backgroundColor": "rgba(112, 255, 115, 0.73)"
    },
    {
      "isToday": false,
      "distance": 1,
      "unit": "weeks",
      "direction": "after",
      "backgroundColor": "rgba(255, 254, 112, 0.91)"
    },
    {
      "isToday": false,
      "distance": 3,
      "unit": "days",
      "direction": "after",
      "backgroundColor": "rgba(255, 215, 112, 0.91)"
    },
    {
      "distance": 1,
      "unit": "days",
      "direction": "after",
      "isToday": true,
      "backgroundColor": "rgba(255, 112, 112, 0.91)"
    },
    {
      "isToday": false,
      "distance": 3,
      "unit": "hours",
      "direction": "after",
      "backgroundColor": "rgba(252, 15, 15, 0.88)",
      "color": "rgba(255, 255, 255, 1)"
    },
    {
      "distance": 1,
      "unit": "days",
      "direction": "after",
      "isBefore": true,
      "backgroundColor": "rgba(64, 83, 255, 0.14)"
    },
    {
      "isToday": false,
      "distance": 1,
      "unit": "months",
      "direction": "before",
      "backgroundColor": "rgba(64, 163, 255, 0.53)"
    },
    {
      "distance": 3,
      "unit": "months",
      "direction": "before",
      "backgroundColor": "rgba(28, 232, 103, 0.56)"
    }
  ],
```
Create Kanban cases
![image](https://github.com/mgmeyers/obsidian-kanban/assets/90405292/260a68af-14ee-4113-a85a-0685616c8d32)


### Relevant errors (if available)

_No response_

### Operating system

Windows

## Investigation Notes
<!-- To be filled during triage -->

## Fix Attempts
<!-- Document any attempted fixes -->
